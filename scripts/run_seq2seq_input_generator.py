...
import transformers
from trainer_seq2seq_input_generator import QuestionAnsweringSeq2SeqTrainer
...
require_version(
    "datasets>=1.8.0", "To fix: pip install -r examples/pytorch/question-answering/requirements.txt"
)
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        def foramt_squad_dataset(example):
            return {
        ...
        raw_datasets = raw_datasets.map(
            foramt_squad_dataset, batched=False, num_proc=data_args.preprocessing_num_workers
        )
    ...
    def preprocess_squad_batch(
        ...
        def generate_input(_question, _context):
            if not data_args.chat:
                return " ".join(["context:", _context.lstrip()])
            else:
                return " ".join(["persona:", _context.lstrip()])

        inputs = [
            generate_input(question, context) for question, context in zip(questions, contexts)
        ]
        targets = [question.strip() for question in questions]
    ...
    def preprocess_validation_function(examples):
        ...
        # [Yongrae] We don't assume multiple features here.
    ...
    def post_processing_function(
        ...
        references = [
            {"id": ex["id"], "answers": {"text": [ex[question_column]], "answer_start": [0]}}
            for ex in examples
        ]
    ...
    if training_args.do_predict:
        ...
        output_data = [
            {"id": prediction["id"], "question": prediction["prediction_text"]}
            for prediction in results.predictions
        ]
        with open(data_args.output_file, "w") as output_file:
            for output_datum in output_data:
                input_datum = predict_examples.filter(lambda d: d["id"] == output_datum["id"])[0]
                if "answer" in input_datum:
                    del input_datum["answer"]
                output_line = json.dumps({**input_datum, **output_datum})
